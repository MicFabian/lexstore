...
@Component({
            ...
            @case ('playground') {
              <div class="panel" style="padding:18px;max-width:720px">
                ...
                <div class="row" style="gap:8px;margin-bottom:14px">
                  <lx-btn variant="primary" icon="WandSparkles" [disabled]="!srcText()" (clicked)="translate(false)">Translate</lx-btn>
                  ...
                  <tbody>
                    @for (r of requests(); track r.id) {
                      <tr class="trow" style="cursor:default">
                        ...
                        <td>
                          @if (r.status === 'error') {
                            <span style="color:var(--lx-danger);font-size:12.5px">{{ r.errorMessage }}</span>
                          } @else {
            ...
            @case ('cache') {
              <div class="panel">
                <div class="panel__head">
                  <h2>Cache</h2>
                  <div class="spacer"></div>
                  <lx-search placeholder="Search source or target" [value]="cacheQuery()" [width]="240" (changed)="onCacheSearch($event)" />
                  <button class="btn btn--subtle btn--sm" style="color:var(--lx-danger)" (click)="clearAll()">
                ...
                <div class="panel" style="max-width:640px">
                  ...
                  <div style="padding:18px;display:flex;flex-direction:column;gap:18px">
                    ...
                    <div class="field">
                      ...
                      <textarea class="textarea" rows="2" [value]="draft().tone || ''" (input)="patch({ tone: $any($event.target).value })" placeholder="e.g. Keep it concise; never translate the word 'Lexstore'."></textarea>
                    </div>
                    <div class="row">
                    ...
                    <div><lx-btn variant="primary" (clicked)="saveSettings()">Save settings</lx-btn></div>
                  </div>
                </div>
              }
            }
  ...
  styles: `
    ...
    .statcell + .statcell { border-left: 1px solid var(--lx-line); }
