#!/usr/bin/env node
/**
 * Lexstore CLI — pull and push translations from your terminal.
 *
 *   lexstore projects
 *   lexstore languages   --project <code>
 *   lexstore pull        --project <code> --lang <code> [--out file.json] [--status translated,proofread]
 *   lexstore push        --project <code> --lang <code> --in file.json [--status translated]
...
function fail(msg) {
  process.stderr.write(`lexstore: ${msg}\n`);
  process.exit(1);
}
...
async function resolveProject(code) {
  ...
  if (!project) fail(`no project with code "${code}" (try: lexstore projects)`);
  return project;
}
...
const HELP = `lexstore — pull and push translations from your terminal
...
Env:
  LEXSTORE_API   API base URL (default ${API})
`;
...
async function main() {
  ...
  switch (command) {
    ...
    case '-h':
      ...
      return;
    default:
      fail(`unknown command "${command}" (try: lexstore help)`);
  }
}
